...
public class CouponService {
	...
	public Page<Record> paginate(int page,int limit,Kv kv) {
		SqlPara sPara=Db.getSqlPara("coupon.queryList", kv);
	...
	public void delectByIds(String string) {
		...
		for (int i = 0; i < array.size(); i++) {
			Db.deleteById("shop_coupon",array.get(i));
		}
	}
	public Ret publish(String string) {
		System.out.println("这个是接收的值"+string);
		PublishModel pModel=FastJson.getJson().parse(string, PublishModel.class);
		Kv kv = FastJson.getJson().parse(string, Kv.class);
        ...
        if (1==sendType) { // 下发用户逗号分割
			...
			List<ShopUserCoupon> list=new ShopUserCoupon().findAll();
			Map<String, String> codeMap = new HashMap<String, String>();
			for (ShopUserCoupon userModel : list) {
				codeMap.put(userModel.getSourceKey(), "1");
			}
			for (int i = 0; i < Integer.parseInt(kv.getStr("number")); i++) {
				String inviteCode = RandomUtil.randomString(10);//生成邀请码
				inviteCode = getInviteCode(inviteCode, codeMap);
				ShopUserCoupon shopUserCoupon=new ShopUserCoupon();
				shopUserCoupon.setCouponId(couponId);
				shopUserCoupon.setCouponNumber("1");
				shopUserCoupon.setAddTime(new Date());
				shopUserCoupon.setSourceKey(inviteCode);
				shopUserCoupon.setCouponStatus(1L);
				shopUserCoupon.save();
			}
		}
        //todo
	...
	public String getInviteCode(String inviteCode, Map<String, String> codeMap) {
		String checkCode = "";
		if (codeMap.get(inviteCode) != null) {
			checkCode = RandomUtil.randomString(10);
			// 递归设置子节点
			getInviteCode(checkCode, codeMap);
		} else {
