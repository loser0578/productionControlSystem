$(function () {
    $("#jqGrid").Grid({
        url: '../sys/role/list',
        colModel: [
            {label: '角色ID', name: 'role_id', index: "role_id", key: true, hidden: true},
            {label: '角色名称', name: 'role_name', index: "role_name", width: 75},
            {label: '所属部门', name: 'deptName', width: 75},
            {label: '备注', name: 'remark', width: 100},
            {
                label: '创建时间', name: 'create_time', index: "create_time", width: 80, formatter: function (value) {
...
var vm = new Vue({
    ...
    data: {
        ...
        showList: true,
        title: null,
        role: {deptId: '', deptName: '',role_name:'',remark:''},
        ruleValidate: {
            roleName: [
    ...
    methods: {
        ...
        getRole: function (roleId) {
            Ajax.request({
                ...
                successCallback: function (r) {
                    ...
                    //勾选角色所拥有的菜单
                    var menuIds = r.menuList;
                    console.log("menuIds"+JSON.stringify(menuIds))
                    for (var i = 0; i < menuIds.length; i++) {
                        var node = menu_ztree.getNodeByParam("menu_id", menuIds[i].menu_id);
                    ...
                    for (var i = 0; i < deptIds.length; i++) {
                        var node = data_ztree.getNodeByParam("dept_id", deptIds[i]);
        ...
        saveOrUpdate: function (event) {
            ...
            for (var i = 0; i < nodes.length; i++) {
                menuIdList.push(nodes[i].menu_id);
            }
        ...
        getMenuTree: function (roleId) {
            ...
            Ajax.request({
                ...
                async: true,
                successCallback: function (r) {
                	console.log("r.menuList"+JSON.stringify(r.menuList))
        ...
        getDept: function () {
            ...
            Ajax.request({
                ...
                successCallback: function (r) {
                    ...
                    var node = dept_ztree.getNodeByParam("dept_id", vm.role.dept_id);
                    if (node != null) {
